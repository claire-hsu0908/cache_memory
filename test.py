...
class CodeCheck:
    ...
    @staticmethod
    def check_java_math_used(filename):
        """
        Attempt to find Math in Java programs (doesn't fully parse, so can be fooled, but should be good enough).
        Returns true if it's found, else false.
        Quietly returns false if file can't be opened.
        """
        try:
            ...
            pattern = re.compile('/\\*.*?\\*/', flags=re.DOTALL) # eliminte multi-line comments
            str1 = re.sub(pattern," ",str1)
            return "Math." in str1 # if Math.* still remains
        except Exception as e:
            verbose_print("check_java_math_used: %s" % e)
            return False
...
class Suite(JSONWrapper):
    ...
    def check_suite_level_penalties(self):
        ...
        if self.has('penalty_java_math_or_modulo'):
            penalty_info = self['penalty_java_math_or_modulo']
            this_penalty = penalty_info['penalty']
            target = penalty_info['file']
            verbose_print("%s: Checking for modulo and math.h" % target)
            # Checking for modulo in c/java is the same.
            if CodeCheck.check_c_modulus_used(target):
                penalty *= this_penalty
                message += "%s: It appears that modulo was used; this will multiply the score by %.2f\n" % (target, this_penalty)
            elif CodeCheck.check_java_math_used(target):
                penalty *= this_penalty
                message += "%s: It appears that java.lang.Math was included; this will multiply the score by %.2f\n" % (target, this_penalty)
